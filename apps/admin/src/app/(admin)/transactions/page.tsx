...
import { DollarSign, Search, CreditCard, CheckCircle2, Clock, Eye, X, QrCode } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">
          Theo dõi lịch sử chuyển khoản tự động qua SePay 10.000 VNĐ mở khóa sách số E-Book và thu phí quá hạn.
        </p>
      </div>
                ...
                <tr key={tx.orderCode} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="p-3.5 text-right">
                    <button
                      ...
                      className="font-bold text-amber-600 hover:text-amber-700 underline flex items-center gap-1 justify-end cursor-pointer"
                    >
                      <Eye className="h-3.5 w-3.5" /> Xem Mã QR
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl p-6 space-y-4 shadow-2xl border border-slate-200 text-center">
            ...
            <div className="space-y-3 text-xs">
              ...
              <p className="text-[11px] text-slate-400">Giao dịch đã gạch nợ thành công tự động qua cổng SePay VietinBank.</p>
            </div>

            <div className="pt-2 border-t">
              <Button onClick={() => setSelectedTx(null)} className="w-full bg-slate-900 text-white font-bold">Đóng</Button>
            </div>
          </div>
        </div>
      )}
